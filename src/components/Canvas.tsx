...
export default function Canvas({ collapse, onToggle }: SidebarProps) {
  ...
  const [grids, setGrids] = useState<[string,string,boolean][]>(getGridsIdandNames());
  ...
  return (
    <>
      <div className="fixed">
        ...
        <div className="">
          {!collapse && <Sidebar collapse={collapse} onClose={onToggle}  grids={grids} setActiveGrid={setActiveGrid} key={`datas.map((data)=>{
            if (data.active){
              return data.id
            }
           })`}/>}
